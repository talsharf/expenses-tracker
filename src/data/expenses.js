export const expenseData = Array.from({ length: 100 }, (_, index) => {
  ...
  if (['Dining Out', 'Entertainment', 'Shopping'].includes(type)) {
    max = 150; // Smaller amounts for frequent expenses
  } else if (['Rent', 'Traveling'].includes(type)) {
    min = 800;
    max = 2500; // Larger amounts for major expenses
  } else if (['Utilities', 'Healthcare', 'Education', 'Transportation', 'Groceries'].includes(type)) {
      min = 50;
      max = 400;
  }
